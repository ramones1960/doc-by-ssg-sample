...
各 SSG の Markdown 資産を **Pandoc + LuaLaTeX** で直接 PDF 変換したサンプルを [`docs/pdf-samples/`](./docs/pdf-samples/) に収録しています。
ヘッダー・フッター（章タイトル・ページ番号・日付）付きの A4 PDF が生成されます。
再生成コマンドは [`docs/pdf-samples/README.md`](./docs/pdf-samples/README.md) を参照してください。
